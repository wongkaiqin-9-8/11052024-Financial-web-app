...
def prediction():
    return(render_template("prediction.html")) #r frontend to receive from backend

@app.route("/dbs_price",methods=["GET","POST"])
def dbs_price():
    q = float(request.form.get("q")) #backend
    return(render_template("dbs_price.html",r=(q*-50.6)+90.2))

@app.route("/generate_text",methods=["GET","POST"]) #get from backend, post to frontend
def generate_text():
    return(render_template("generate_text.html"))

@app.route("/text_result_makersuite",methods=["GET","POST"])
def text_result_makersuite():
    q = request.form.get("q") #backend
...
if __name__ == "__main__": #ensure that its u again
    app.run()
